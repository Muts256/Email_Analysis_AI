...
- **Domains/URLs:** pinnacle-group.com — no malicious URLs present. Attack is entirely social engineering based
- **Attachments:** None
...
### Social Engineering Indicators
- Authority impersonation — sender presents as CEO of an external organisation
...
**Severity: CRITICAL**
Justification: Impersonates a trusted internal function, targets the entire organisation, and the macro-enabled attachment has the potential to deliver a range of high-impact payloads. A single user enabling macros could result in network-wide compromise.
...
This suggests the threat actor conducted reconnaissance on TechNova Ltd prior to launching the campaign — identifying named staff, their roles, and the organisation's email infrastructure.
...
Case Study 002 passed every technical check. SPF passed. DKIM passed. DMARC passed with a reject policy. No links. No attachments. By every technical measure, this was a legitimate email. Only behavioural and contextual assessment — the urgency, the isolation tactic, the unavailability of the sender, the unusual financial request — reveals it as an attack. Without that layer of analysis, this email walks straight through.
